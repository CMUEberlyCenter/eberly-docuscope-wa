...
# English
en_extra_stop_words = ['therefore', 'however', 'thus', 'hence', '\'s', 'mr.', 'mrs.', 'ms.', 'dr.', 'prof.']
en_pronouns = ['it',  'i',    'we',   'you',   'he',  'she',  'they',
...
def setLanguageModel(lang, model=NLP_MODEL_DEFAULT):
    ...
    try:
        ...
        stop_words = list(nlp.Defaults.stop_words)   # get a list of language specific stop words
...
def get_text_preserve_inline(element, inline_tags=None):
    if inline_tags is None:
        inline_tags = ['strong', 'em', 'b', 'i', 'u', 'span', 'a', 'code', 'mark', 'small', 'sub', 'sup']
    
    # Get the inner HTML
    html = element.decode_contents()

    # Remove block-level tags but keep their content
    block_tags = ['div', 'p', 'h[1-6]', 'section', 'article', 'header', 'footer', 'nav', 'aside']
...
class DSDocument():
    ...
    def _process_document(self) -> List[DocumentElement]:
            ...
            if tag.name == 'p':
                ...
                for parent in tag.parents:
                    if parent.name in ['td', 'th']:
                        return False
            return tag.name in ['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'table', 'ul', 'ol', 'li']
        ...
        all_elements = self.soup.find_all(element_not_in_table_cell)
    ...
    def _process_element(self, html_element, position: int) -> Union[DocumentElement, None]:
        ...
        if tag_name in ['p']:
            ...
            return DocumentElement(
                ...
                position=position,
                styles=styles,
                metadata={'tag': tag_name, 'src': src, 'alt': alt_text, 'caption': caption}
            )
    ...
    def processSent(self, sent, start=0):
        ...
        if type(sent) != str:           # sent is a spacy object
            ...
            for token in spacy_doc:
                ...
                w = (pos, token.text, lemma, left, token.dep_, '', None, False, word_pos, None)
                res.append(w)
    ...
    def analyzeSent(self, sent_dict, content_type):
        ...
        res['NOUN_CHUNKS'] = [ {'text': np.text, 'start': np.start, 'end': np.end} for np in spacy_doc.noun_chunks]
    ...
    def processDoc(self):
        ...
        for elem in self.elements:         # for each paragraph in the file.
            ...
            if elem.content_type in [ContentType.PARAGRAPH, ContentType.LIST, ContentType.HEADING, ContentType.LISTITEM]:
                p = elem.text
            else:
                para_dict['text']  = ""
                continue                      
            ...
            for sent_dict in para_dict['sentences']:
                ...
                if elem.content_type == ContentType.HEADING:
                    sent_dict['text_w_info'], NPs, word_pos = self.processSent(sent_dict['text'], start=word_pos+1)
                else:
                    sent_dict['text_w_info'], NPs, word_pos = self.processSent(sent_dict['sent'], start=word_pos+1)

                sent_dict['sent_analysis']      = self.analyzeSent(sent_dict, elem.content_type)
                sent_dict['lemmas']             = listLemmas(sent_dict['text_w_info'])  # list lemmas in the sentence
    ...
    def toHtml(self, topics=[], para_pos=-1, font_info=default_font_info):
        ...
        for elem in self.elements:
            ...
            if elem.content_type not in [ContentType.PARAGRAPH, ContentType.LISTITEM]:
                ...
                tag = self.soup.find(id=f"p{para_id}")              # find the tag with the id = elem.para_id
                tag.clear()                  
            elif elem.content_type == ContentType.LISTITEM:
                pcount = elem.position
                para_id = elem.para_id                
                tag = self.soup.find(id=f"li{pcount}")             # find the tag with the id = elem.position
                tag = self.soup.find(id=f"p{pcount}")              # find the tag with the id = elem.position 
                tag.clear()                                        # clear the content

            if elem.content_type == ContentType.LISTITEM and prev_content_type == ContentType.LISTITEM:
            ...
            for sent in elem.data['sentences']:                # for each sentence within this element.
                total_words = len(sent['text_w_info'])         # get the total # of words
                is_combo = False
                word_count = 0

                # open <span> tag
                html_str += '<span id="p{0}s{1}" class="sentence" data-ds-paragraph="{0}" data-ds-sentence="{1}"> '.format(para_id, scount)
                ...
                while word_count < total_words:
                    ...
                    if lemma in topics and (w[POS] == 'NOUN' or w[POS] == 'PRP'):  # if w[LEMMA] in topics:
                        # we need to remove periods.
                        topic = unidecode.unidecode(w[LEMMA].replace('.',''))
                        html_word = '<span class="word" data-ds-paragraph="{}" data-ds-sentence="{}" data-topic="{}">{}</span> '.format(para_id, scount, topic, word) 
                    else:
                        html_word = word
                            ...
                            if next_w[WORD] in no_space_patterns:
                                ...
                                while True:
                                    ...
                                    if temp_w[LEMMA] in topics and (temp_w[POS] == 'NOUN' or temp_w[POS] == 'PRP'):
                                        topic = unidecode.unidecode(w[LEMMA].replace('.',''))
                                        html_temp_word = '<span class="word" data-ds-paragraph="{}" data-ds-sentence="{}" data-topic="{}">{}</span> '.format(para_id, scount, topic, temp_w[WORD])
                                    else:
    ...
    def getLocalTopicalProgData(self, selected_paragraphs):  
        ...
        for pos in selected_paragraphs:            
            ...
            if e.content_type in [ContentType.PARAGRAPH, ContentType.HEADING, ContentType.LIST]:
    ...
    def getGlobalTopicalProgData(self, sort_by=TOPIC_SORT_APPEARANCE):
        ...
        for elem in self.elements:
            if elem.content_type not in [ContentType.PARAGRAPH, ContentType.LIST, ContentType.HEADING]:
                continue
    ...
    def getSentStructureData(self):
        ...
        for elem in self.elements:
            ...
            if elem.content_type in [ContentType.PARAGRAPH, ContentType.LISTITEM, ContentType.HEADING]:
                p = elem.data
                para_id = elem.para_id
                ...
                for s in p['sentences']:             # for each sentence
                    if len(s['text']) == 1 and \
                        ...
                        bSkipPunct = False
                    res.append(tuple([para_id, s_count, s['sent_analysis'], s, bSkipPunct]))
