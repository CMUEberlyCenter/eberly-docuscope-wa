...
from localization.NLP import Locale
...
class TopicFilter(Enum):
    """
    TopicFilter is used to determine which words should be considered when generating the topic clusters.

    For example, if TopicFilter.LEFT is selected, only the words on the left side of the main verb will be considered for topic clustering. This allows users to focus on the subjects and themes of the sentences, which are often found on the left side of the main verb.
    """
...
class TopicSort(Enum):
    """TopicSort is used to determine how the topics should be sorted when generating the topic clusters."""
...
def is_skip(elem, left_count: int, topic_filter: TopicFilter) -> bool:

    # if theme_only == True:
    if topic_filter == TopicFilter.LEFT:
...
def contains_html_tags(text: str) -> bool:
    """Check if the text contains HTML like tags."""
...
def get_text_preserve_inline(
    element: Tag,
    inline_tags: Optional[List[str]] = None,
    remove_with_attrs: Optional[Dict[str, List[str]]] = None,
) -> str:
    """
...
def extract_and_replace_images(html_string: str) -> Tuple[str, Dict[str, str]]:
    """
...
def get_table_rows_and_cols(table_element: Tag) -> Optional[Tuple[int, int]]:
    """Get the number of rows and columns in a table element."""
...
def get_image_caption(img_element: Tag) -> str:
...
def element_not_in_table_cell(tag: Tag) -> bool:
    """Check if the tag is not inside a table cell (td or th). This is used to exclude elements that are inside table cells when processing the document."""
...
class DSDocument:
    ...
    def _process_element(
        self, html_element: Tag, position: int
        ...
        if self.locale is None or self.locale.nlp is None:
        ...
        if tag_name in ["p"]:
            ...
            data["sentences"] = []
            parsed_para = self.locale.nlp(text)
        ...
        if tag_name in ["img"]:
            ...
            caption = get_image_caption(html_element)
    ...
    def _get_table_summary(self, table_element: Tag) -> str:
        """Get a brief summary of table content"""
        return self.locale.tableSummary(get_table_rows_and_cols(table_element))
    ...
    def getRealParaCount(self) -> int:
        return self.getNumParagraphs()

    def getSentCount(self) -> int:
        return sum(len(elem.data["sentences"]) for elem in self.elements)
    ...
    def processSent(self, sent, start=0):
        ...
        def processHeading(heading):
            nonlocal word_pos
            res = []
            if self.locale.nlp is None:
                return res, ()
            spacy_doc = self.locale.nlp(heading)
            ...
            for token in spacy_doc:
                ...
                if self.locale.isTerminalPunctuation(token.text):
                    pos = "PUNCT"
        ...
        if not isinstance(sent, str):  # sent is a spacy object
            ...
            for token in spacy_doc:
                ...
                if {} is not None:  # DSDocument.user_defined_synonyms is not None:
                    if token.pos_ in ("NOUN", "PROPN"):  # if the token is a noun
                        t = token.text.lower()  # original spelling
                        lemma = None  # DSDocument.user_defined_synonyms.get(t, None)  #

                        if lemma is None:
                            t = token.lemma_.lower()  # SpaCy's lemma
                            lemma = (
                                None  # DSDocument.user_defined_synonyms.get(t, None)
                            )
                        ...
                        t = token.text.lower()
                        lemma = None  # DSDocument.user_defined_synonyms.get(
                        # t, None
                        # )  # e.g., He

                        if lemma is None:
                            lemma = self.locale.getPronounLemma(t)  # e.g., his
                            if lemma is None:
                                lemma = token.lemma_.lower()
                        else:
                            token.pos_ = "NOUN"
                        ...
                        if lemma is None:
                            lemma = self.locale.getPronounLemma(t)
                            if lemma is None:
                                lemma = token.lemma_.lower()
                        else:
                            token.pos_ = "NOUN"
    ...
    def analyzeSent(self, sent_dict, content_type):
        ...
        for data in sent_data:
            ...
            if data[DEP] == "ROOT":
                ...
                if self.locale.isBeVerb(data[WORD]):
                    res["BE_VERB"] = True
        ...
        spacy_doc = self.locale.nlp(text) if self.locale.nlp is not None else None
        ...
        if advcl_root is not None:
            ...
            for w in advcl_root.lefts:
                if count == 0:
                    start = w.i
                    if (
                        self.locale.shouldIgnoreAsFirstWord(w.text.lower())
                        or start != 0
                    ):
                        bIgnore = True
                        break
    ...
    def processDoc(self):
        ...
        if self.locale.nlp is None:
            logging.error("Error: language model not loaded!")
            return []
        ...
        def listLemmas(sent):
            ...
            for w in sent:
                if (
                    w[POS] is not None
                    and not self.locale.isStopWord(w[LEMMA])
                    and w[POS] in ("NOUN", "PRP")
                ):
    ...
    def loadFromTxt(self, a_text: str):
            ...
            for sent in elem.data[
                ...
                while word_count < total_words:
                    ...
                    if word_count < total_words - 1:
                        ...
                        if self.locale.isEndQuote(word) and html_str[-1] == " ":
                            # if w[WORD] is a right side quotes, and the current html_str string ends
                            # with a space, remove the space.
                            html_str = html_str[:-1]

                        if self.locale.isStartQuote(word) or word in hyphen_slash:
                        ...
                        elif not self.locale.isStartQuote(next_w[WORD]):
                            # next_w is a punct, which is not on one of the left quotes

                            temp_word_count = 0
                            if self.locale.shouldNotHaveSpace(next_w[WORD]):
                                ...
                                if (
                                    ...
                                    if self.locale.shouldNotHaveSpace(
                                        temp_w[WORD]
                                    ) or self.locale.isEndQuote(temp_w[WORD]):
                                        ...
                                        POS
                                    ] == "PUNCT" and not self.locale.isEmphasisPunctuation(
                                        temp_w[WORD]
                                    ):
                                        combo_word = combo_word + temp_w[WORD]
                                        word_count = temp_word_count
                                ...
                                if temp_next_w is not None:
                                    ...
                                    if self.locale.shouldNotHaveSpace(
                                        temp_next_w[WORD]
                                    ) or self.locale.isEndQuote(temp_next_w[WORD]):
                                        is_space = False

                                    elif self.locale.isTerminalPunctuation(
                                        temp_next_w[WORD]
                                    ) and not self.locale.isEmphasisPunctuation(
                                        temp_next_w[WORD]
                                    ):
                                        is_space = False
                            ...
                            elif self.locale.isEndQuote(next_w[WORD]):
                                is_space = False
                                ...
                                POS
                            ] == "PUNCT" and self.locale.isTerminalPunctuation(
                                next_w[WORD]
                            ):
                                is_space = False
                            else:
                        ...
                        elif self.locale.isEndQuote(word):
                            if html_str[-1] == " ":
    ...
    # def getSentencesWithTopics(self, topics: List[str]) -> List[Tuple[int, int, str]]:
    ...
    def getLocalTopicalProgData(self, selected_paragraphs: List[int]):
        ...
        for elem in selected_elem_list:
            ...
            for s in p["sentences"]:
                ...
                bSkipPunct = bool(
                    len(s["text"]) == 1
                    and (
                        s["text"] in hyphen_slash
                        or self.locale.isStartQuote(s["text"])
                        or self.locale.isEndQuote(s["text"])
                        or self.locale.isEmphasisPunctuation(s["text"])
                    )
                )
    ...
    def getGlobalTopicalProgData(self, sort_by=TopicSort.APPEARANCE):
        ...
        all_lemmas.sort(key=lambda tup: tup[2])  # sort by the order of appearance.
        if sort_by == TopicSort.LEFT_COUNT:
            all_lemmas.sort(
    ...
    #     DSDocument.generateGlobalVisData(self, min_topics=2, max_topic_sents=1, sort_by=TopicSort.APPEARANCE)
    #
    ...
    #     topic_filter = TopicFilter.LEFT_RIGHT
    ...
    #                 if topic_filter == TopicFilter.ALL:
    ...
    #             is_tc = False # DSDocument.isUserDefinedSynonym(topic)
    ...
    def generateGlobalVisData(
        self, min_topics=2, max_topic_sents=1, sort_by=TopicSort.APPEARANCE
    ):
        """
        This method returns a python dictionary that contains the data that are needed for
        ...
        for ci in range(ncols):  # for each topic entry,
            ...
            if topic is not None:  # topic exists
                if not self.isLocalTopic(topic):
                ...
                # Count how many times the topic appears on the left side of the main verb.
                # if DSDocument.isUserDefinedSynonym(topic):  # topic is a topic cluster.
                #     true_left_count = sent_filter[topic]["left_count"]
                #     count = sent_filter[topic]["count"]
                #     l_count = sent_filter[topic]["left_count"]
                #     count = max(count, 2)  # at least 2
                # else:  # topic is not a topic cluster
                true_left_count = sent_filter[topic]["left_count"]
                if topic_filter == TopicFilter.ALL:  # rarely used.
                    count = sent_filter[topic]["count"]
                    l_count = sent_filter[topic]["left_count"]
                else:  # default
                    l_count = sent_filter[topic]["left_count"]
            ...
            is_tc = False  # DSDocument.isUserDefinedSynonym(
            # topic
    ...
    def getHtmlSents(self, data):
        ...
        def generate_html(sent_data):
            ...
            for wpos, token in enumerate(tokens):
                ...
                if (
                    self.locale.isEndQuote(w)
                    or self.locale.isTerminalPunctuation(w)
                    or w == "%"
                    or self.locale.shouldNotHaveSpace(w)
                ):
                    if html_str[-1] == " ":
                ...
                if tag.startswith("</b>"):
                    if (
                        not self.locale.isEndQuote(w)
                        and not self.locale.isTerminalPunctuation(w)
                        and w != "%"
                        and not self.locale.shouldNotHaveSpace(w)
                    ):
                        html_str += " "
                ...
                if not self.locale.isStartQuote(w) and w not in hyphen_slash:
                    html_str += " "
