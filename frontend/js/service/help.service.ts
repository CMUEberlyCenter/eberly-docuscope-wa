...
const GETTING_STARTED_URL = new URL(
  '../assets/getting_started.html',
  import.meta.url
);
const TROUBLESHOOTING_URL = new URL(
  '../assets/troubleshooting.html',
  import.meta.url
);
...
export const [useHelp, help$] = bind(
  fromFetch<string>(HELP_URL.toString(), {
...
export const showGettingStarted = (show: boolean) =>
  show_getting_started.next(show);
export const [useShowGettingStarted, showGettingStarted$] = bind(
  show_getting_started,
  false
);
export const [useGettingStarted, gettingStarted$] = bind(
  fromFetch<string>(GETTING_STARTED_URL.toString(), {
    selector: (response) => response.text(),
  }).pipe(
    shareReplay(1),
    catchError((err) => {
...
export const showTroubleshooting = (show: boolean) =>
  show_troubleshooting.next(show);
export const [useShowTroubeshooting, showTroubleshooting$] = bind(
  show_troubleshooting,
  false
);
export const [useTroubleshooting, troubleshooting$] = bind(
  fromFetch<string>(TROUBLESHOOTING_URL.toString(), {
    selector: (response) => response.text(),
  }).pipe(
    shareReplay(1),
    catchError((err) => {
