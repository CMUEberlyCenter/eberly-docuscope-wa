...
import { getCourseId, launch } from "../../DocuScopeTools";
...
class InstuctorView extends EberlyLTIBase {
  ...
  onLaunch(_e) {
    ...
    if (this.state.selected == "" || this.state.selected == "global") {
      console.log(
        "There is no file selected for this assignment. Please upload or select a file below first"
      );
      return;
    }
  ...
  dropHandler(event) {
    ...
    if (typeof this.state.settings !== "undefined") {
      if (event.dataTransfer.items) {
        console.log(
          "Uploading " + event.dataTransfer.items.length + " files ..."
        );
        ...
        // Use DataTransfer interface to access the file(s)
        console.log(
          "Uploading " + event.dataTransfer.files.length + " files ..."
        );
    ...
    if (this.state.selected == "") {
      return (
        <div className="loaderbar">
          <button id="launcher" className="inline_button">
  ...
  generateUploadElement() {
    return (
      <div id="filecontainer" htmlFor="file" className="dropcontainer">
        <div className="box">
          <input
            type="file"
            id="file"
          ...
          <td>
            <label>
              <input
                ...
                type="radio"
                value={file.id}
                checked={this.state.selected === file.id}
                onChange={(e) => this.onFileSelectionChanged(e, file.id)}
    ...
    return (
      <div tabIndex={0} className="ltiapp">
        <div className="versionlabel">
          Eberly DocuScope Write & Audit version: {this.state.version}
        </div>
