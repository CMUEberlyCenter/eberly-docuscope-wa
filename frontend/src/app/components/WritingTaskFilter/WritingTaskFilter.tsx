...
export const WritingTaskFilter: FC<WritingTaskFilterProps> = ({ update, ...props }) => {
  ...
  useEffect(() => {
    if (indeterminateRef.current) {
      ...
      indeterminateRef.current.indeterminate = !active.isSupersetOf(keys) && active.intersection(keys).size > 0;
    }
  ...
  return (
    <div {...props}>
      <div className="d-flex align-items-center gap-1 mb-2">
        ...
        <Form.Select aria-label={t('select_task.select')} value={category}
          onChange={(evt) => setCategory(evt.target.value)}>
          {Object.entries(keywords).map(([key,]) => (
            <option value={key} key={key}>
              {key === 'ALL' ? t('select_task.all_category') : key.replace(/^\w+:\s*/, '')}
            </option>
          ))}
        </Form.Select>
      </div>
      <Form>
        <Form.Check ref={indeterminateRef} type="checkbox" label={t("select_task.select_all")} checked={new Set(activeKeywords).isSupersetOf(new Set(categoryKeywords))}
          onChange={() => setActiveKeywords(new Set(activeKeywords).isSupersetOf(new Set(categoryKeywords)) ? [] : categoryKeywords)} />
        <hr />
        {categoryKeywords.map((key, i) => (
          <Form.Check type="checkbox" label={key} key={`${key}-${i}`} checked={activeKeywords.includes(key)} onChange={() =>
            setActiveKeywords(
              activeKeywords.includes(key) ? activeKeywords.filter(active => active !== key) : [...activeKeywords, key]
            )} />
        ))}
      </Form>
    </div>
