...
export const WritingTaskRulesTree: FC<RuleTreeProps> = ({ includeTitle, select, leafOnly, className, ...props }) => {
  const { t } = useTranslation();
  const writingTask = useWritingTask();
  const [selected, setSelected] = useState<Rule | null>(null);
  const onSelect = useCallback((rule: Rule) => {
    if (!leafOnly) {
      setSelected(rule === selected ? null : rule);
    }
  }, [leafOnly, selected]);
  const onSelectLeaf = useCallback((rule: Rule) =>
    setSelected(rule === selected ? null : rule), [selected]);
  useEffect(() => {
    if (select) {
      select(selected);
    }
  }, [select, selected])
  const cn = classnames(className, "d-flex flex-row align-items-stretch gap-3 position-relative");
  return (<div {...props} className={cn}>
    <div className="d-flex flex-column align-items-start writing-task-tree h-0 w-100 overflow-auto">
      ...
      {writingTask?.rules.rules.map((rule) => (
        <div key={rule.name} aria-expanded="true">{/* TODO?: add some indicator of sub-tree selection possibly only on non-expanded. */}
          <div className={classnames('d-flex')}>
            ...
            <div aria-selected={selected === rule}
              className={classnames("fw-bold", "flex-grow-1",
          ...
          <div className="expanded">
            <ul>
              {rule.children.map((cluster) => (
                <li key={cluster.name}
                  className={classnames('pointer', selected === cluster ? 'text-bg-dark' : '')}
