...
import { Descendant, Editor, createEditor, Element as SlateElement, Transforms } from "slate";
...
const Leaf: FC<RenderLeafProps> = ({ children, leaf, attributes }) => (<span {...attributes}
  style={{
    fontWeight: ('bold' in leaf) && leaf.bold ? 'bold' : 'normal',
    textDecoration: (('underline' in leaf) && ('strikethrough' in leaf) && leaf.underline && leaf.strikethrough) ? 'underline line-through' : ('underline' in leaf && leaf.underline ? 'underline' : ('strikethrough' in leaf && leaf.strikethrough ? 'line-through' : undefined)),
...
const isBlockActive = (editor: Editor, format: string): boolean => {
  const [match] = Editor.nodes(editor, { match: n => !Editor.isEditor(n) && SlateElement.isElement(n) && n.type === format });
  return !!match;
}
...
const MarkButton: FC<{ format: Markings, children: ReactNode }> = ({ format, children }) => {
      ...
      <main className="d-flex overflow-none h-100 flex-column">
        <ButtonToolbar aria-label="Editor Tools">
          <ButtonGroup>
            ...
            <Form.Select aria-lable="Block format" size="sm" defaultValue={'paragraph'}
              onChange={(e) => {
            ...
            <MarkButton format="bold"><FontAwesomeIcon icon={faBold} /></MarkButton>
            <MarkButton format="italic"><FontAwesomeIcon icon={faItalic} /></MarkButton>
            <MarkButton format="underline"><FontAwesomeIcon icon={faUnderline} /></MarkButton>
            <MarkButton format="strikethrough"><FontAwesomeIcon icon={faStrikethrough} /></MarkButton>
          </ButtonGroup>
          <div className="ms-3">
