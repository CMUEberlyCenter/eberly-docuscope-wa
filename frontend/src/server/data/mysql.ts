 ...
 */
export async function findWritingTaskIdByAssignment(
  assignmentId: string
 ...
 */
export async function findWritingTaskByAssignment(
  assignmentId: string
): Promise<WritingTask> {
  const [rows] = await pool.query<IFileData[]>(
 ...
 */
// REMOVE prompts now stored separate from assignment.
