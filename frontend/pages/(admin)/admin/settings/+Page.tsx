    ...
    <Card>
      ...
      <Card.Body>
        ...
        <ListGroup className="mb-3">
          ...
          <ListGroup.Item>
            Word Count Limit:{' '}
            <strong>{settings?.word_count_limit.toLocaleString() ?? 'No limit'}</strong>
          </ListGroup.Item>
          <ListGroup.Item>
            {t('tool.button.prose.tooltip')} {t('tool.tab.generate')} Tool:{' '}
            <strong>{settings?.scribe && settings?.notes2prose ? 'Enabled' : 'No'}</strong>
          </ListGroup.Item>
          <ListGroup.Item>
            {t('tool.button.bullets.tooltip')} {t('tool.tab.generate')} Tool:{' '}
            <strong>{settings?.scribe && settings?.notes2bullets ? 'Enabled' : 'No'}</strong>
          </ListGroup.Item>
          <ListGroup.Item>
            {tr('review:civil_tone.title')} {tr('review:title')} Tool:{' '}
            <strong>{settings?.scribe && settings?.civil_tone ? 'Enabled' : 'No'}</strong>
          </ListGroup.Item>
          <ListGroup.Item>
            {tr('review:credibility.title')} {tr('review:title')} Tool:{' '}
            <strong>{settings?.scribe && settings?.credibility ? 'Enabled' : 'No'}</strong>
          </ListGroup.Item>
          <ListGroup.Item>
            {tr('review:expectations.title')} {tr('review:title')} Tool:{' '}
            <strong>{settings?.scribe && settings?.expectations ? 'Enabled' : 'No'}</strong>
          </ListGroup.Item>
          <ListGroup.Item>
            {tr('review:lines_of_arguments.title')} {tr('review:title')} Tool:{' '}
            <strong>{settings?.scribe && settings?.lines_of_arguments ? 'Enabled' : 'No'}</strong>
          </ListGroup.Item>
          <ListGroup.Item>
            {tr('review:logical_flow.title')} {tr('review:title')} Tool:{' '}
            <strong>{settings?.scribe && settings?.logical_flow ? 'Enabled' : 'No'}</strong>
          </ListGroup.Item>
          <ListGroup.Item>
            {tr('review:paragraph_clarity.title')} {tr('review:title')} Tool:{' '}
            <strong>{settings?.scribe && settings?.paragraph_clarity ? 'Enabled' : 'No'}</strong>
          </ListGroup.Item>
          <ListGroup.Item>
            {tr('review:professional_tone.title')} {tr('review:title')} Tool:{' '}
            <strong>{settings?.scribe && settings?.professional_tone ? 'Enabled' : 'No'}</strong>
          </ListGroup.Item>
          <ListGroup.Item>
            {tr('review:prominent_topics.title')} {tr('review:title')} Tool:{' '}
            <strong>{settings?.scribe && settings?.prominent_topics ? 'Enabled' : 'No'}</strong>
          </ListGroup.Item>
          <ListGroup.Item>
          ...
          <ListGroup.Item>
            ...
            <strong>{settings?.scribe && settings?.sources ? 'Enabled' : 'No'}</strong>
          </ListGroup.Item>
          <ListGroup.Item>
