...
import "../../../src/app/components/Editor/CustomEditor.scss";
import { FormatDropdown } from "../../../../src/app/components/Editor/FormatDropdown";
import { MarkButton } from "../../../../src/app/components/Editor/MarkButton";
import { FileDownload } from "../../../../src/app/components/FileDownload/FileDownload";
import { FileUpload } from "../../../../src/app/components/FileUpload/FileUpload";
import ToolCard from "../../../../src/app/components/ToolCard/ToolCard";
import WritingTaskDetails from "../../../../src/app/components/WritingTaskDetails/WritingTaskDetails";
...
const CustomEditor: FC = () => {
  const { task, userInfo, resource } = useData<Data>();
  ...
  const saveAs = useCallback(async () => {
    if (content) {
      const blob = await Packer.toBlob(
        serializeDocx(content, task, userInfo?.name)
      );
      if (
  ...
  }, [content, userInfo, resource, task, upload]);
  ...
  return (
    ...
    >
      ...
      >
        <main className="d-flex overflow-none flex-column my-1">
          ...
          >
            ...
            <div className="ms-auto">
              ...
              <Button variant="light" onClick={() => setShowTask(!showTask)}
                title={t("tool.button.view.title", {
                  title: task?.rules.name,
                })}>
                {task?.rules.name}
              </Button>
              <WritingTaskDetails show={showTask} onHide={() => setShowTask(false)} writingTask={task} />
            </div>
          </ButtonToolbar>
          <Editable
        ...
        <ToolCard hasSelection={selection} writingTask={task} />
      </Split>
      <FileUpload
        show={showUpload}
